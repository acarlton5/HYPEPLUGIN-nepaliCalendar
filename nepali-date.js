...
function findBsDateFromTotalNumberOfDays(year, days) {
    let month = 0;
    for (let i = 0; i < 12; i++) {
...
function formatBSDateVertical(bsDateStr, format, showDevanagari = false) {
    ...
    if (format.indexOf("YYYY") !== -1 || format.indexOf("YY") !== -1) {
        verticalParts.push(yy);
    }

    if (format.indexOf("Month") !== -1) {
        verticalParts.push(shortMonth);
    } else if (format.indexOf("MM") !== -1) {
        verticalParts.push(mm);
    }

    if (format.indexOf("dd") !== -1) {
        verticalParts.push(dd);
    }

    if (verticalParts.length === 0) {
